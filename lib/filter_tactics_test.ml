  ...
  [%expect {|
    alist(select([p_name], part) as s0,
      atuple([filter((strpos(p_name, param1) > 0), ascalar(s0.p_name)),
              select([p_partkey, p_mfgr, p_brand, p_type, p_size, p_container,
                      p_retailprice, p_comment, o_orderkey, o_custkey,
                      o_orderstatus, o_totalprice, o_orderdate, o_orderpriority,
                      o_clerk, o_shippriority, o_comment, l_orderkey, l_partkey,
                      ...
                      l_comment, ps_partkey, ps_suppkey, ps_availqty,
                      ps_supplycost, ps_comment, s_suppkey, s_name, s_address,
                      s_nationkey, s_phone, s_acctbal, s_comment, n_nationkey,
